...
The extraction script detects *that* an image exists (see the bullet above) but never its content — there's no OCR. Non-image content — vector-drawn diagrams/shapes, non-image embedded OLE objects, text boxes, and form fields — isn't detected at all, by presence or content. Inspect any of this directly (open the file, or extract images via `python-docx`'s `document.part.rels` / `openpyxl`'s `worksheet._images`) when a question depends on it.
